...
/**
 * Shared doubt-thread UI used by both the student (/dashboard/doubts/[id])
 * and creator (/creator/doubts/[id]) routes. The caller owns the layout
 * chrome and passes `backHref` / `backLabel` so the "Back" affordance
 * matches the persona that navigated here.
 */
...
export function DoubtThread({
  ...
  const threadQuery = trpc.doubt.byId.useQuery({ doubtId });
      ...
      {canClose && (
        <div className="flex justify-end">
          <Button
            ...
            onClick={() => closeMutation.mutate({ doubtId })}
          >
